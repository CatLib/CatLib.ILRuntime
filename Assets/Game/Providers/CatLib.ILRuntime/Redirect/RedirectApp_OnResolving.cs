...
namespace CatLib.ILRuntime.Redirect
{
    ...
    {
        private static void RegisterOnResolving()
        {
            mapping.Register("OnResolving", 1, 1, new string[]
            {
                "System.Action`1[TWhere]"
            }, OnResolving_TWhere_Action1);
            mapping.Register("OnResolving", 1, 1, new string[]
            {
                "System.Action`2[CatLib.IBindData,TWhere]"
            }, OnResolving_TWhere_Action2);
        }

        // public static IContainer OnResolving<TWhere>(Action<TWhere> closure)
        private static StackObject* OnResolving_TWhere_Action1(ILIntepreter intp, StackObject* esp, IList<object> mStack,
            CLRMethod method, bool isNewObj)
        {
            ...
            var result = App.OnResolving((container, instance) =>
            {
        ...
        private static StackObject* OnResolving_TWhere_Action2(ILIntepreter intp, StackObject* esp, IList<object> mStack,
            CLRMethod method, bool isNewObj)
        {
            var genericArguments = method.GenericArguments;
            if (genericArguments == null || genericArguments.Length != 1 || method.ParameterCount != 1)
