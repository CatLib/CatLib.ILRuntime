...
namespace CatLib.ILRuntime.Redirect
{
    ...
    {
        private static void RegisterOnRelease()
        {
            mapping.Register("OnRelease", 1, 1,new string[]
            {
                "System.Action`1[TWhere]"
            }, OnRelease_TWhere_Action1);
            mapping.Register("OnRelease", 1, 1, new string[]
            {
                "System.Action`2[CatLib.IBindData,TWhere]"
            }, OnRelease_TWhere_Action2);
        }

        // public static IContainer OnRelease<TWhere>(Action<TWhere> closure)
        private static StackObject* OnRelease_TWhere_Action1(ILIntepreter intp, StackObject* esp, IList<object> mStack,
            CLRMethod method, bool isNewObj)
        {
            ...
            var result = App.OnRelease((container, instance) =>
            {
        ...
        private static StackObject* OnRelease_TWhere_Action2(ILIntepreter intp, StackObject* esp, IList<object> mStack,
            CLRMethod method, bool isNewObj)
        {
            var genericArguments = method.GenericArguments;
            if (genericArguments == null || genericArguments.Length != 1 || method.ParameterCount != 1)
